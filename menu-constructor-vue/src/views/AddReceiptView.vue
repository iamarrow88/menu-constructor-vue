    ...
    <form action="">
      ...
      <div class="my-select">
        ...
        <div class="my-input">
          ...
          <input
            ...
            name="breakfast"
            id="breakfast"
            v-model="receipt.mealType.breakfast"
          />
        </div>
        <div class="my-input">
      ...
      <div class="my-textarea">
        <label for="ingredients-area"
          >Список ингридиентов в формате "Ингридиент"
          <select v-model="separator">
            <option>-</option>
            <option>:</option>
            <option>^</option>
          </select>
          "количество". Будьте внимательны: После второго тире в строке все теряется.</label
        >
        <textarea
          name="ingredients-area"
          id="ingredients-area"
          cols="30"
          rows="5"
          v-model="rawIngredients"
        ></textarea>
      </div>
      <div class="my-textarea">
        <label for="how-to-cook-area">Способ приготовления</label>
        <textarea
          name="how-to-cook-area"
          id="how-to-cook-area"
          cols="30"
          rows="10"
          v-model="receipt.howToCook"
        ></textarea>
      </div>
      <div class="my-tags">
...
export default {
  ...
  methods: {
    async saveReceipt() {
      const areIngredientsFormatted = formatIngredients(this.rawIngredients, this.separator);
      if (areIngredientsFormatted) {
        this.receipt.ingredients = areIngredientsFormatted;
      } else {
        throw Error('Не удалось сохранить рецепт, так как есть проблема с ингредиентами')
      }

      if (
        stringIsNotEmpty(this.receipt.name) &&
